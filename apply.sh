...
original_name="flask_trial"
original_urlname="flask-trial"
original_description="Awesome flask_trial created by niski1996" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_trial "${name}"
...
then
    ...
    echo "then run 'flask_trial' or 'python -m flask_trial'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_trial' or 'python -m flask_trial' or 'poetry run flask_trial'"
fi
