...
- 🎯 Entry points to execute your program using `python -m <flask_trial>` or `$ flask_trial` with basic CLI argument parsing.
...
---
# flask_trial Flask Application

Awesome flask_trial created by niski1996
...
```bash
git clone https://github.com/niski1996/flask-trial flask_trial
cd flask_trial
make install
```
...
```bash
flask_trial create-db   # run once
flask_trial populate-db  # run once (optional)
flask_trial add-user -u admin -p 1234  # ads a user
flask_trial run
```
